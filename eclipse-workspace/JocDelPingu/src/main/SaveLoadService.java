...
import config.CryptoUtil;
import entity.Entity;
import entity.Player;
import entity.Seal;
import ConnectionDDBB.BBDD;
...
public class SaveLoadService {
    ...
    public static boolean loadGame(String gameId) {
        try {
            ...
            gamePanel.GameSetupConfig.setLoadedGame(true);
            gamePanel.GameSetupConfig.setLoadedBoardState((List<String>) state.get("board"));
            gamePanel.GameSetupConfig.setLoadedTurnIndex(((Number) state.get("currentTurn")).intValue());
            ...
            for (Map<String, Object> pMap : playersList) {
                ...
                if (pMap.containsKey("lastEventType")) {
                    board.EventManager.EventType type = board.EventManager.EventType.valueOf((String) pMap.get("lastEventType"));
                    String detail = (String) pMap.get("lastEventDetail");
                    int newPos = ((Number) pMap.get("lastEventPos")).intValue();
                    p.setLastEvent(new board.EventManager.EventResult(type, detail, newPos));
                }
                
                players.add(p);
            }
            gamePanel.GameSetupConfig.setPlayers(players);

            if (state.containsKey("seal")) {
                gamePanel.GameSetupConfig.setSealEnabled(true);
                gamePanel.GameSetupConfig.setLoadedSealState((Map<String, Object>) state.get("seal"));
            } else {
                gamePanel.GameSetupConfig.setSealEnabled(false);
                gamePanel.GameSetupConfig.setLoadedSealState(null);
            }
